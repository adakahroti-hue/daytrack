-- ============================================================================
-- KONSOLIDASI GOAL: drop struktur lama (jika ada) lalu create struktur v2.
-- Idempoten: aman dijalankan berulang kali.
-- Gantikan migration 05 & 06 — cukup jalankan file ini saja.
-- ============================================================================

-- 1) Drop tabel lama (goal v1 + goal_utama + relasinya) bila masih ada
DROP TABLE IF EXISTS public.goal_langkah CASCADE;
DROP TABLE IF EXISTS public.goal_utama CASCADE;
DROP TABLE IF EXISTS public.goal CASCADE;

-- 2) Create tabel goal v2
...
CREATE POLICY "Users can CRUD own goal_step" ON public.goal_step
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.goal_milestone m JOIN public.goal g ON g.id = m.goal_id
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'goal_updated_at') THEN
    CREATE TRIGGER goal_updated_at BEFORE UPDATE ON public.goal FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'goal_milestone_updated_at') THEN
    CREATE TRIGGER goal_milestone_updated_at BEFORE UPDATE ON public.goal_milestone FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'goal_step_updated_at') THEN
    CREATE TRIGGER goal_step_updated_at BEFORE UPDATE ON public.goal_step FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
END $$;
