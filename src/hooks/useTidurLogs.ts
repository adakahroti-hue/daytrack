...
export function useTidurLog(tanggal: string) {
  return useQuery({ queryKey: ["tidur_logs", tanggal], queryFn: () => getTidurLog(tanggal), enabled: !!tanggal })
}
export function useTidurLogRange(startDate: string, endDate: string) {
  return useQuery({ queryKey: ["tidur_logs", "range", startDate, endDate], queryFn: () => getTidurLogRange(startDate, endDate), enabled: !!startDate && !!endDate })
}
export function useTidurStats(startDate: string, endDate: string) {
  return useQuery({ queryKey: ["tidur_logs", "stats", startDate, endDate], queryFn: () => getTidurStats(startDate, endDate), enabled: !!startDate && !!endDate })
}
export function useUpsertTidurLog() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data: TidurLogFormData) => upsertTidurLog(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tidur_logs"] }) }
  })
}
