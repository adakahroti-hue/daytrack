...
export function useWaterLogRange(startDate: string, endDate: string) {
  return useQuery({ queryKey: ["minum_air_logs", "range", startDate, endDate], queryFn: () => getWaterLogRange(startDate, endDate), enabled: !!startDate && !!endDate, placeholderData: keepPreviousData })
}
