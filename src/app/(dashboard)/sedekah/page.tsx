...
import { Calendar, CalendarDays, HandCoins, Check, X, Trash2, MessageCircle, Pencil } from 'lucide-react'
...
// Revisi 2 (batch 5): alasan tidak bersedekah → disimpan di kolom alasan_tidak
type SedekahReason = 'sibuk' | 'malas' | 'tidak_fokus'

const SEDEKAH_REASONS: { value: SedekahReason; label: string }[] = [
...
export default function SedekahPage() {
  ...
  const { data: logs = [], isLoading, error } = useSedekahLogRange(startDate, endDate)
  useRealtime({
    table: 'sedekah',
    filter: `tanggal=gte.${startDate},tanggal=lte.${endDate}`,
    queryKeys: [['sedekah', 'range', startDate, endDate]],
  })

  const upsertSedekahLog = useUpsertSedekahLog()
  const deleteSedekahLog = useDeleteSedekahLog()

  // Map tanggal -> entry
  const logMap = useMemo(() => {
    const map: Record<string, SedekahLogEntry> = {}
    for (const l of logs as SedekahLogEntry[]) map[l.tanggal] = l
    return map
  }, [logs])
  ...
  const handleSetStatus = async (tanggal: string, status: 'sudah' | 'belum', reasonValue?: SedekahReason) => {
    await upsertSedekahLog.mutateAsync({
      tanggal,
      status,
  ...
  const analyticsEntries = useMemo(() => {
    return (logs as SedekahLogEntry[]).map(l => ({
      tanggal: l.tanggal,
  ...
  const handleClear = async (tanggal: string) => {
    ...
    if (entry) await deleteSedekahLog.mutateAsync(entry.id)
  }
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
          ...
          <thead className={cn('hidden sm:table-header-group sticky top-0 z-20 bg-white')}>
            <tr className={cn('border-b', TABLE_BORDER)}>
              ...
              <th className={cn('px-2 sm:px-3 py-2 text-center font-semibold text-slate-700 border-r min-w-[120px] sm:min-w-[150px]', TABLE_BORDER)}>
                <div className="flex items-center justify-center gap-1">
                  <HandCoins className="h-3.5 w-3.5 text-green-500" />
                  Status
                </div>
              </th>
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button size="icon" aria-label="Ubah status sedekah" className="h-6 w-6 p-0 bg-slate-600 hover:bg-slate-700 text-white">
      ...
      <StatusAnalytics
        ...
        difficultyTitle="Sedekah Terbanyak"
        reasonTitle="Alasan Tak Bersedekah"
        missedNoun="terlewat"
      />
    </div>
  )
