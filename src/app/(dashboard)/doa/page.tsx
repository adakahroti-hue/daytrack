...
import { Calendar, CalendarDays, Heart, Check, X, Trash2, MessageCircle, Pencil } from 'lucide-react'
                        ...
                        <div className="space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              ...
                              {isMissed && entry?.alasan && (
                                <div className="mt-1 text-[11px] text-slate-500">{entry.alasan}</div>
                              )}
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button size="icon" aria-label="Ubah status doa" className="h-6 w-6 p-0 bg-slate-600 hover:bg-slate-700 text-white">
                                  ...
                                  {isMissed && (
                                    <>
                                      ...
                                      {DOA_REASONS.map(r => (
                                        <DropdownMenuItem
                                          key={r}
                                          onClick={() => handleSetStatus(dateStr, 'belum', r)}
                                          className="flex items-center gap-2"
                                        >
                                          {entry?.alasan === r ? <Check className="h-4 w-4 text-green-600" /> : <span className="w-4 h-4" />}
                                          {r}
                                        </DropdownMenuItem>
                                      ))}
                                    </>
